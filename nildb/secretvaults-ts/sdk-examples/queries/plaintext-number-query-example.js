...
import { findContactsByExactAgeQuery, findContactsByAgeRangeQuery, findContactsOlderThanQuery, findContactsYoungerThanQuery, calculateAverageAgeQuery, groupContactsByAgeRangeQuery, findOldestAndYoungestQuery, getContactsByAgeWithPaginationQuery, calculateAgeStatisticsQuery, } from '../query-examples-number.js';
...
async function plaintextNumberQueryExample() {
    try {
        ...
        console.log('📌 This example demonstrates querying on plaintext number fields in SecretVaults.');
        console.log('   We show various query operations using the age field as an example.\n');
        ...
        async function runQueryAndWaitForResults(queryId, variables = {}) {
            const runRequest = {
                _id: queryId,
                variables,
            };
        ...
        console.log('Contacts aged 20-30:', JSON.stringify(ageRangeResult, null, 2));
        // Demo 3: Find older than
        console.log(`\n⬆️  Demo 3: Find Contacts Older Than`);
        const olderThanResult = await runQueryAndWaitForResults(queryIds.olderThan, {
            minAge: 50,
        });
        console.log('Contacts older than 50:', JSON.stringify(olderThanResult, null, 2));
        // Demo 4: Find younger than
        console.log(`\n⬇️  Demo 4: Find Contacts Younger Than`);
        const youngerThanResult = await runQueryAndWaitForResults(queryIds.youngerThan, {
            maxAge: 20,
        });
        console.log('Contacts younger than 20:', JSON.stringify(youngerThanResult, null, 2));
        // Demo 5: Calculate average age
        console.log(`\n🧮 Demo 5: Calculate Average Age`);
        const avgAgeResult = await runQueryAndWaitForResults(queryIds.avgAge);
        console.log('Average age statistics:', JSON.stringify(avgAgeResult, null, 2));
        ...
        console.log(`\n🎯 Demo 7: Find Oldest and Youngest`);
        const oldestYoungestResult = await runQueryAndWaitForResults(queryIds.oldestYoungest);
        console.log('Oldest and youngest ages:', JSON.stringify(oldestYoungestResult, null, 2));
