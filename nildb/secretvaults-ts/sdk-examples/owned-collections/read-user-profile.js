...
async function readUserProfile() {
    try {
        ...
        if (Array.isArray(error) && error.some((e) => e?.error?.status === 401)) {
            console.log('User profile not found. Create owned data first to establish profile.');
            return null;
        }
...
readUserProfile()
    .then((result) => {
    if (result) {
        console.log('User profile retrieved successfully', JSON.stringify(result, null, 2));
    }
    process.exit(0);
})
    .catch((error) => {
