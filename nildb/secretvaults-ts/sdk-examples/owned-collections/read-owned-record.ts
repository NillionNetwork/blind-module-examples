...
import { ReadDataRequestParams } from '@nillion/secretvaults';

async function readOwnedRecord(collectionId: string, recordId: string) {
  try {
...
if (!collectionId || !recordId) {
  ...
  if (!recordId) console.error('- NILLION_OWNED_RECORD_ID');
  process.exit(1);
}
...
readOwnedRecord(collectionId, recordId)
  .then((result) => {
    console.log(
      'Successfully read owned record',
