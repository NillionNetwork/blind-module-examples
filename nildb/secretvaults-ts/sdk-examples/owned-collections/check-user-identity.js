...
export async function checkUserIdentity() {
    try {
        ...
        const nodeDetails = await Promise.all(userClient.nodes.map(async (node) => {
            try {
...
checkUserIdentity()
    .then((result) => {
    console.log('User identity information retrieved successfully', JSON.stringify(result, null, 2));
    process.exit(0);
})
    .catch((error) => {
