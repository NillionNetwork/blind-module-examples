...
import { DeleteDocumentRequestParams } from '@nillion/secretvaults';

async function deleteOwnedRecord(collectionId: string, documentId: string) {
  try {
    const userClient = await initSecretVaultUserClient();

    const deleteParams: DeleteDocumentRequestParams = {
...
setTimeout(() => {
  deleteOwnedRecord(collectionId, documentId)
    .then((result) => {
      console.log(
        'Successfully deleted owned data',
