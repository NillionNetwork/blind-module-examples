...
async function fullOwnedDataExample() {
    ...
    try {
        ...
        console.log('✅ Delegation token created (expires in 60 minutes)', tokenString);
        ...
        const createResponse = await userClient.createData(tokenString, createDataRequest);
        ...
        if (referencesResponse.data && referencesResponse.data.length > 0) {
            console.log(`✅ Found ${referencesResponse.data.length} owned record(s):`);
        ...
        console.log(`   https://collection-explorer.nillion.com/collections/${collectionId}`);
        return {
            collectionId,
            recordId,
