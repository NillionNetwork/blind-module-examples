...
async function createOwnedData(collectionId, delegationToken, granteeDid) {
    try {
        ...
        const createResponse = await userClient.createData(delegationToken, createDataRequest);
...
if (!collectionId || !delegationToken || !granteeDid) {
    ...
    if (!delegationToken)
        console.error('- NILLION_DELEGATION_TOKEN (from create-delegation-token script)');
    if (!granteeDid)
        console.error('- NILLION_GRANTEE_DID (DID to grant access to, e.g., builderClient DID)');
    process.exit(1);
}
createOwnedData(collectionId, delegationToken, granteeDid)
    .then((result) => {
    console.log('Successfully created owned data', JSON.stringify(result, null, 2));
    process.exit(0);
})
    .catch((error) => {
